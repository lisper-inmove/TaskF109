...
def main():
    ...
    parser.add_argument("-p", "--ports", nargs="+", type=int, default=[8888],
                       help="监听的端口列表 (默认: 8888)")
    parser.add_argument("--host", default="0.0.0.0",
                       help="监听地址 (默认: 0.0.0.0)")
    parser.add_argument("--config", help="配置文件路径")
    parser.add_argument("--stats-interval", type=int, default=10,
                       help="统计信息打印间隔 (秒，默认: 10)")
    parser.add_argument("--custom-handler", action="store_true",
                       help="使用自定义消息处理器")
    ...
    try:
        ...
        if args.stats_interval > 0:
            stats_thread = threading.Thread(
                target=print_stats, 
                args=(manager, args.stats_interval),
                daemon=True
            )
            stats_thread.start()
